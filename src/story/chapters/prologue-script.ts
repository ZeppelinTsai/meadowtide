...
export const PROLOGUE_SCRIPT: Record<string, DialogueLine[]> = {
  ...
  tour: [
    ...
    mayor(
      "「靠近廣場的第一間店就是雜貨店。營業時間是上午十點到下午四點，星期一公休。」",
    ),
    mayor(
  ...
  farming: [
    ...
    "[村長短暫停頓]",
    mayor(
      "「……不好意思。人上了年紀，清理這些東西比想像中累。我只來得及替你整理這一塊。」",
    ),
    "[主角連忙搖頭，冒著汗表示自己不是在責怪她]",
    mayor(
      "「哈哈，不用急著道歉。接下來的地方，就交給你按照自己的步調整理吧。」",
    ),
    mayor("「站在田地上按下 E，就能播種。來，把這九格都種滿看看。」"),
    "[玩家完成九格播種]",
    mayor("「沒錯，就是這樣。」"),
    mayor(
      "「這片田連著池塘的引水系統，播種後不需要每天澆水。等作物成熟，就可以直接收成了。」",
    ),
    "[鏡頭帶過田地旁的水渠]",
    mayor("「那邊是動物小屋。以前養過不少牛、羊和雞，不過現在已經空了。」"),
    mayor(
      "「等你準備好，我可以從雜貨店聯絡本島的店家。挑選並付款後，動物會在隔天搭船送到牧場。」",
    ),

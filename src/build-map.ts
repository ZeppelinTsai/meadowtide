      ...
      export function buildMap(mapName) {
        ...
        if (mapName === "house") {
          ...
          function groundSlab(xStart, xEnd, height) {
            ...
            const depth =
              rows * TILE + northExtension + SOUTH_TERRAIN_EXTENSION;
          ...
          for (
            ...
            const inCorridor =
              z >= RAMP_CORRIDOR_MIN_Z && z <= RAMP_CORRIDOR_MAX_Z;
            if (inCorridor) {
          ...
          const southRampDepth =
            rows - southRampStartZ + SOUTH_TERRAIN_EXTENSION;
          [
            [rampX, PLATEAU_Y * 0.75],
            [rampX + 1, PLATEAU_Y * 0.5],
            [rampX + 2, PLATEAU_Y * 0.25],
          ...
          const northOceanStartX = map.tiles[0].indexOf(9);
          if (northOceanStartX > lowlandX) {
            const minX = lowlandX - 0.5;
          ...
          const northCliffEndX =
            LAYOUT.coast.rampX + LAYOUT.coast.rampWidth - 0.5;
          ...
          for (let x = northCliffStartX; x < northCliffEndX; x += 1) {
            ...
            const wallHeight =
              PLATEAU_Y + 0.12 + hash2(centerX * 4.7, 2.3) * 0.28;
          ...
          const sgGeo = new THREE.PlaneGeometry(
            northSeaWidth,
            northSeaDepth,
            30,
            8,
        ...
        map.tiles.forEach((row, z) => {
          row.forEach((tile, x) => {
            ...
            // tile === 9（海）不逐格建置，迴圈結束後另外蓋成一整片海面，才能做波浪動畫
              ...
              if (insideArea(LAYOUT.restArea) || insideArea(LAYOUT.garden))
                return;
              if (x >= 14 && x <= 16) return; // 坡道走廊留乾淨，不要長裝飾把路擋亂
              const r = hash2(x * 3.1, z * 7.7);
              const gy = groundY(x, z);
              if (r < 0.1) {
                const m = makeGrassTuft(
                  x + (r - 0.5) * 0.4,
                  z + (r - 0.5) * 0.4,
                  r,
                );
        ...
        if (mapName === "livingArea") {
          ...
          if (validZ.length) {
            ...
            const FISH_COUNT = 6;
            const safeSeaMinX =
              Math.max(...westXByZ.filter((v) => v !== null)) + 0.7;
            for (let i = 0; i < FISH_COUNT; i++) {
              const s1 = hash2(i * 3.1, i * 7.7),
                s2 = hash2(i * 5.3, i * 2.9);
              const cx =
                safeSeaMinX + s1 * Math.max(0.2, maxX - 0.7 - safeSeaMinX);
          ...
          for (let gx = PASTURE.minX; gx <= PASTURE.maxX; gx++) {
            for (let gz = PASTURE.minZ; gz <= PASTURE.maxZ; gz++) {
              ...
              for (let i = 0; i < 3; i++) {
                ...
                const jx =
                  gx + (hash2(gx + i * 11.3, gz * 2.1 + i) - 0.5) * 0.94;
                const jz =
                  gz + (hash2(gz + i * 13.7, gx * 3.4 + i) - 0.5) * 0.94;
                const broadGrowth =
                  1.5 +
                ...
                tuft.userData.growth = Math.max(
                  0.08,
                  Math.min(2.98, broadGrowth),
                );
          ...
          if (lakeCells.length) {
            ...
            for (let i = 0; i < LAKE_FISH_COUNT; i++) {
              ...
              const cx =
                centerX + Math.cos(angle) * radiusX * (0.12 + s1 * 0.22);
              const cz =
                centerZ + Math.sin(angle) * radiusZ * (0.12 + s2 * 0.22);
              const mesh = makeFishProp(100 + i);
              const scale =
                LAKE_FISH_SCALE * (0.8 + hash2(100 + i, 31.4) * 0.42);
              mesh.scale.setScalar(scale);

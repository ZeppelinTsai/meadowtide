...
function targetForGather(nodeId:string):WorldTarget|null{if(!hasTool("dualAxe"))return null;const entry=gatherNodeMeshes.find(e=>e.nodeId===nodeId&&e.map===gameState.currentMapName);const node=[...WOOD_NODES,...STONE_NODES].find(n=>n.id===nodeId);if(!entry||!node||node.collected)return null;return{id:"gather:"+nodeId,object:entry.group,radius:1.2,actions:[legacyAction(node.kind,node.kind==="wood"?"\u780d\u6750":"\u63a1\u77f3")],getPosition:()=>node.collected?null:{x:node.x,z:node.z},isValid:()=>hasTool("dualAxe")&&!node.collected&&entry.group.visible};}
...
function targetForStove():WorldTarget|null{
  if(gameState.currentMapName!=="house"||!canUsePrologueKitchen())return null;
  const stove=(MAPS.house.furniture||[]).find(item=>item.type==="stove");
  if(!stove)return null;
  return{id:"house-stove",object:stoveTargetObject,radius:1.25,actions:[legacyAction("cook","做菜")],getPosition:()=>gameState.currentMapName==="house"?{x:stove.x,z:stove.z}:null,isValid:()=>gameState.currentMapName==="house"&&canUsePrologueKitchen()};
}
...
function allTargets(){
  ...
  {const t=targetForStove();if(t)list.push(t);}

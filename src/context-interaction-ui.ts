...
import { requestPlayerNavigation, cancelPlayerNavigation, onNavigationDestinationChanged } from "./player-navigation";
import { gatherNodeMeshes } from "./scene-registries";
...
function blocked(){return !gameState.player || gameState.titlePresentationActive || gameState.cutsceneActive || isCameraAdjustModeActive() || isInventoryOpen() || dialogQueue.length>0 || Boolean(activeChoice) || gameState.fishingState!=="idle";}
...
export function initContextInteraction(){
  ...
  renderer.domElement.addEventListener("pointerdown",event=>{ activePointers.add(event.pointerId); if(activePointers.size>1){down=null;return;} if(event.button!==0||blocked())return; down={x:event.clientX,y:event.clientY,pointerId:event.pointerId}; });

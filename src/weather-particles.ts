...
const activeWeatherParticleCount = (baseCount: number) =>
  scaleCountForWorldBounds(baseCount, WEATHER_BOUNDS, BASE_WEATHER_AREA);
    ...
    [autumnLeafEffect, autumnLeafForegroundEffect].forEach(
      (layer, layerIndex) => {
        ...
        for (let i = 0; i < layerCount; i++) {
          ...
          const gust =
            Math.sin(
              gameState.effectElapsed * (0.48 + seed * 0.7) + seed * 23,
            ) *
            (0.75 + layerIndex * 0.4);
          layer.positions[base] = wrapWeatherParticle(
    ...
    gameState.currentWeather === "storm"
      ? Math.pow(
          Math.max(
            0,
            Math.sin(
              gameState.effectElapsed * 0.73 +
                Math.sin(gameState.effectElapsed * 0.19) * 2.1,
            ),
          ),
          34,
        )

...
export function makeSeasonStarGroup(seasonIndex) {
  ...
  if (seasonIndex === 1) {
    ...
    for (let i = 0; i < MILKY_WAY_MIST_COUNT; i++) {
      ...
      const spread = (hash2(i * 7.13, 5.8) + hash2(i * 2.37, 23.1) - 1) * 0.12;
      const p = starSkyPoint(
  ...
  const sparkleMaterials = sparklePositions.map((sparklePoints, phaseGroup) => {
...
export function getMeteorActivity() {
  ...
  return {
    shower,
    minPerHour:
      (shower?.minPerHour ?? METEOR_CONFIG.normalMinPerHour) * weatherFactor,
    maxPerHour:
      (shower?.maxPerHour ?? METEOR_CONFIG.normalMaxPerHour) * weatherFactor,
  };
}
...
export function scheduleNextMeteor(reset = false) {
  ...
  if (
    !reset &&
    activity.shower &&
    gameState.elapsed >= gameState.meteorBurstCooldownUntil
  ) {
    const burstChance =
    ...
    if (Math.random() < burstChance) {
      ...
      gameState.meteorBurstCooldownUntil =
        gameState.elapsed + 3.5 + Math.random() * 3;
      gameState.nextMeteorAt = gameState.elapsed + 0.38 + Math.random() * 0.72;
      return;
    }
  }
  ...
  if (activity.shower?.phase === "peak") {
    ...
    interval = -Math.log(Math.max(0.0001, 1 - Math.random())) / eventsPerSecond;
...
export function spawnMeteor() {
  ...
  if (showerMeteor) {
    ...
    startY = radiantY + (Math.random() - 0.5) * (skyCeiling - skyFloor) * 0.72;
    ...
    startX = camera.left + marginX + Math.random() * (viewWidth - marginX * 2);
  ...
  const duration =
    ...
    Math.random() * (METEOR_CONFIG.maxDuration - METEOR_CONFIG.minDuration);
  slot.active = true;
  slot.age = 0;
  slot.duration =
...
export function updateMeteors(delta) {
  ...
  meteorPool.forEach((meteor) => {
    ...
    meteor.material.opacity = Math.min(fadeIn, fadeOut) * meteor.brightness;
  });
}
...
export function updateMoon() {
  ...
  const illuminatedFraction = (1 - Math.cos(moonAgeFraction * Math.PI * 2)) / 2;
...
export function updateSunAndClouds(nightFactor) {
  ...
  if (sunSkyGroup.visible) {
    ...
    // 只是整體矮一截
    placeSkyBillboard(sunSkyGroup, sunSkyX, -5.5 + arc * daylight.peak * 0.5);
    ...
    sunCoreMat.opacity = Math.min(1, sunOpacity * 1.05) * skyOnlyVisibility;
    sunGlowMat.opacity = sunOpacity * (0.2 + arc * 0.16) * skyOnlyVisibility;
  ...
  skyClouds.forEach((cloud, index) => {
    ...
    const cloudY =
      ...
      Math.sin(gameState.effectElapsed * 0.08 + cloud.userData.seed * 12) *
        0.32;
...
export function updateSeasonalStars(nightFactor) {
  ...
  const nightVisibility = Math.max(0, Math.min(1, (nightFactor - 0.38) / 0.5));
  ...
  const noonWrappedElapsed =
    (((gameState.elapsed - dayLength / 2) % dayLength) + dayLength) % dayLength;
  ...
  seasonalStarGroups.forEach((group, seasonIndex) => {
    ...
    group.userData.sparkleMaterials.forEach((material, phaseIndex) => {
      ...
      material.opacity = visibility * (0.045 + Math.pow(pulse, 4) * 0.955);
    });
    group.visible = visibility > 0.008;
    group.rotation.z = starPhase * 0.08 + seasonIndex * 0.012;
  });
}
export function updateSkyDome(nightFactor, viewCamera: THREE.Camera = camera) {
  const skyParent =
  ...
  const zenith = SKY_DAY_ZENITH.clone().lerp(SKY_NIGHT_ZENITH, nightFactor);
  const horizon = SKY_DAY_HORIZON.clone().lerp(SKY_NIGHT_HORIZON, nightFactor);

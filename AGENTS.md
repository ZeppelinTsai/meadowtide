...
- 山區西側平台擴充、樹木與採集候選同步規則 → docs/decisions/mountain-west-expansion.md
...
- **人形角色統一高度為 `1.0` 世界單位**（鞋底到頭骨頂端，即頭部幾何本體
  的上緣），接近專案原始低模角色比例，並以村長的
  `makeMayor()` 模型為基準。唯一常數是 `src/humanoid.ts` 的
  `HUMANOID_WORLD_HEIGHT`；新增或修改 `makeHumanoid()`、`makeMayor()`、
  `makeHeroPlayer()` 等模型時，先量鞋底到未縮放頭骨頂端的高度，再用
  `humanoidScale()` 換算。瀏海、髮帽、長髮、呆毛、帽子與頭飾一律不算進
  基準高度；這些只能在統一身高之外額外延伸。角色預覽也必須依相同的人體
  基準框取景，不得用包含頭髮／帽飾的完整 `Box3` 各自縮放。也不要另寫任意縮放倍率，避免
...
- **中長髮(及以上)的角色，頭髮預設要多做一塊蓋住後腦勺，不要只放一頂
  `hairCap`**——這裡已經連續在植物學家、藝術家、女主角(`makeAdventurerHero`)
  身上出現同一種 bug：`hairCap` 常常是壓扁的球(`scale.y` 縮小做出「淺帽」
  的視覺)，扁下去的量會讓球體下緣停在頭部中段，正面被瀏海/五官擋住看不
  出來，但從背後看後頸那一圈會露出膚色，變成「半禿」。修法是另外加一顆
  偏向本地 **+Z**(角色背面；正臉在 -Z)、位置壓低到接近脖子高度、垂直方向
  沒那麼扁的球當 `backHair`/`napeHair`，跟前面的瀏海分開處理——不要想著
  把 `hairCap` 一顆做大做圓去蓋全部，兩次嘗試都因為前後份量抓不準，不是蓋
  過頭(擋到臉/瀏海消失)就是蓋不夠(後腦還是禿)。新角色做中長髮時，做完先
  用 `debug-character.html` 轉到背面看一眼再交出去。短髮/呆毛/放射狀髮束
  (`makeMayor`、`makeHeroPlayer` 男版那類)目前沒踩過這個坑，不強制套用。
  2026-09-04 補充(海神 `makeGoddess` 踩過)：只把 `napeHair` 往 **Y 軸**
  拉高/拉深，正後方(背面鏡頭)看起來接上了，但兩側(耳朵/太陽穴附近，
  3/4 或側面鏡頭)還是會露出一小塊膚色——因為球體在 X 方向的半徑如果比
  `hairCap`/頭部同緯度的半徑窄，側邊就蓋不到。檢查時除了轉到正背面，
  也要轉到 3/4 側面看一次；`napeHair` 的 X 軸 scale 至少要接近甚至超過
  `hairCap` 的 X 軸半徑，不能只顧 Y 方向的深度。
- **`makeLeg()` 的 pivot 高度必須跟 leg/cuff/boot/sole 疊起來的總深度精確
  對上，鞋底(最低的那塊 mesh 下緣)要落在 group 本地座標 y=0，不是隨便一個
  接近 0 的數字**——植物學家跟藝術家都出過鞋子陷進地板的 bug：pivot 定在
  0.44(髖部)，但底下幾層 Y 座標疊加後鞋底停在 y≈-0.14～-0.1675，整隻腳懸
  在地板線以下，肉眼在一般視角看不太出來，要蹲低鏡頭或站在平面上對比才
  發現。這是純算術錯誤(疊加的深度總和沒等於 pivot 高度)，不是縮放或定位
  的問題，`humanoidScale()` 只會等比例縮放，不會把已經跑掉的鞋底拉回 0。
  寫新角色的 `makeLeg()`、或動到既有的腿部座標時，手算一次「pivot.y + 最
  底層 mesh 的 position.y − 該 mesh 半高」是否等於 0(或在 ±0.01 內)；更快
  的做法是直接用 `debug-character.html`——每次切換角色現在會自動算一次
  目前模型最低點的世界座標 Y，貼地會顯示綠色「✓ 鞋底貼地」，陷進地下或
  懸空會標紅/黃字並提示去查 `makeLeg()`，不用再靠肉眼在特定角度才抓到。
  2026-09-04 補充：這個自動檢查曾經誤判主角(男/女)「鞋子陷進地下
  0.070」，查下去發現腿完全沒問題，是檢查工具本身的 bug——
  `makeAdventurerHero()` 右手掛了一根 `visible = false` 的釣竿
  (`parts.rod`，留給之後的釣魚動畫用)，往下延伸到手掌下方 0.56 個單位；
  `THREE.Box3().setFromObject()` 預設不理會 `visible` 旗標，照樣把隱藏
  物件的幾何體算進包圍盒，量出來的「最低點」其實是看不見的竿子，不是
  鞋子。`checkGroundContact()` 已經改成用 `model.traverseVisible(...)`
  自己收集看得見的 mesh 再算包圍盒，只算畫面上真的會出現的東西——以後
  要是幫角色掛什麼 `visible=false` 的隱藏道具/預留物件，這個檢查依然
  準；但如果自己另外寫量測用的 Box3，記得別直接掉進 `setFromObject()`
  的預設行為裡。
...
- **Windows 精確替換的 LF／CRLF 陷阱（2026-08-30 實際發生）**：當
  `apply_patch` 因 ACL 失敗而改用 PowerShell 精確文字替換時，禁止用
  `[Environment]::NewLine` 拼接搜尋字串並假設它等於檔案換行；Windows 上該值
  是 CRLF，但本專案檔案是 LF，`String.Replace()` 找不到目標時會靜默回傳原文，
  後續 TypeScript 建置仍可能成功，造成「回報已修改、磁碟其實仍是舊碼」。替代
  編輯必須使用可容許 `\r?\n`／空白差異的精確 regex，或先偵測檔案換行；寫入前
  必須確認替換結果與原文不同（未命中就非零失敗），寫入後立即用 `rg`／帶行號
  `Get-Content` 讀回關鍵行，再跑 `git diff --check` 與相關建置／測試。只看到
  `npm run build` 成功不能視為替換已套用。

...
- 太陽／月亮反光直接併入既有水面頂點色：以天體世界方向在水平面鏡射後，
  和水面頂點到實際渲染相機的方向計算亮帶；不可為反光另疊透明水面 mesh。
  天氣能見度必須同步衰減反光，第一／第三人稱都讀同一顆渲染相機與天體。
- 夜間星光反射必須在低視角海面仍清楚可辨；取樣密度不可只適合俯視湖面，
  波紋亮度也要保留不閃爍時的最低基線，避免海平線附近整片退回純藍色。

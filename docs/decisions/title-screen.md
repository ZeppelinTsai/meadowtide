...
標題背景是生活區的離線渲染快照，使用第一人稱透視鏡頭
`(18.27, 2.38, 8.01)`、yaw `-7.276`、pitch `0.103`、FOV `65`。季節與
時刻先讀目前遊戲狀態；沒有目前玩家時讀 `savedAt` 最新的存檔，舊存檔沒有
`savedAt` 則以 `elapsed` 回退。快照完成後必須還原共享 gameState。

標題 BGM 分三段：06:00–11:59 白天、12:00–17:59 下午、18:00–05:59 晚上。
離開標題時清除標題音樂優先權，交還地區／天氣／季節音樂。

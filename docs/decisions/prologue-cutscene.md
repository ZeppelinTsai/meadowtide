...
港口介紹結束後，`prologue.ts` 進入 `guidedWalking`：對話必須先完整關閉，村長沿指定路徑領路，主角由玩家自由操作跟上，不再由程式沿村長軌跡自動拖行。村長與玩家距離超過 2.5 格時會停下等待；村長抵達終點後，也要等主角靠近才推進下一段。
同行期間仍維持 `cutsceneActive` 作為事件安全鎖，但 `game-loop.ts` 只為 `guidedWalking` 特別放行方向移動；因此山區入口、其他 touch 換圖與世界互動仍全部停用，只有角色移動本身開放。
目前測試路線為：
...
九格播種是正式玩法閘門：對話結束後暫時令 `cutsceneActive=false`，允許玩家自由
移動與使用快捷背包；`game-loop.ts` 每幀只統計教學田的實際 `cropState`。未滿九格
時鎖住序章日期時間、停用 touch 換圖事件，並讓村長維持原位；滿九格後黑屏、把
兩人復位到 `(14,20)`，再恢復演出與指定同行路徑。序章全部完成後，第一天自由
時間固定由 15:00 開始。

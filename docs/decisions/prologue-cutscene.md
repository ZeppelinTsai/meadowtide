...
序章荒廢農田產生的木材／石材節點須標記為固定障礙，06:00／18:00 的一般採集點刷新不得移除或重排；只有玩家用對應工具親自清除後才消失。序章手動快速存檔的安全檢查點設在 `seekingRod`：更早的引路、播種與室內演出階段一律擋下快速存檔；讀檔須在重建地圖前還原此階段，讓村長同行與港口觸發繼續生效。

...
Start 暫停、R3 切換視角、LB／RB 切換分頁、LT／RT 縮放鏡頭。自由遊玩時
LB 切換視角、RB 在第一人稱拍照；UI 開啟時 LB／RB 仍切換分頁。UI 開啟時
...
- 俯視模式的右搖桿上下同樣連續縮放，速度依搖桿偏移量與幀時間計算；第一人稱模式的右搖桿仍控制視角，不得觸發縮放。LT／RT 在第一人稱改變透視相機 FOV。
- 所有手把縮放統一合成 wheel 事件；俯視鏡頭沿用 setCameraZoom()，第一人稱則調整透視相機 FOV，兩者共用選單守衛。

...
## 2026-09-01 第四輪：粉紅色不夠飽和、蒲公英補描邊放更大、背包縮圖花頭偏上

- **粉紅酢漿草改用更飽和的洋紅粉**（`0xf1789f` → `0xe0327d`），描邊倍率也從共用預設 `1.3` 收到 `1.18`——花瓣本身窄，描邊太粗會把粉色面積吃掉太多，`radialPetalWithOutline()` 現在支援第 6 個參數 `outlineScale` 覆寫預設值，只有這個物種傳自訂值，其他物種維持預設。
- **蒲公英再放大一次並補上描邊**：花瓣從 `(0.07, 0.013)` 再放大到 `(0.082, 0.016)`，改用 `radialPetalWithOutline()`（原本只放大過、沒描邊）。
- **背包/倉庫格子裡花的縮圖偏上**：`inventory-ui.ts` 的 `renderModelThumbnail()` 用 3D bounding box 幾何中心置中，這對「圓潤/塊狀」的道具（石材、木材、魚……）沒問題，但花是「細莖＋只在頂端的花頭」，幾何中心落在莖的中段，視覺上真正吸引目光的花頭反而整個在中心線以上，置中之後看起來就是偏上。修法：只對 `tone === "flower"` 的項目，置中之後再額外往下位移 `size.y * thumbnailScale * 0.14`（用模型自己的高度算比例，不是寫死像素，五個物種尺寸不同也適用），不影響其他任何項目的縮圖。

## 規格決議

以下決議固定為目前規格；不再保留待拍板的猶豫項目：
...
2. **採集消耗體力**：`staminaCost: 0`。本作不設體力條，野花採集固定零成本，不與伐木、採石建立體力扣除耦合。
